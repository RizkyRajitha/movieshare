...
let respormise = (id) => {
  return new Promise((resolve, reject) => {
    fetch(
      `https://api.airtable.com/v0/${AIRTABLEBASEID}/${AIRTABLETABLENAME}/${id}`,
      {
        method: "delete", // make sure it is a "DELETE request"
        // body: JSON.stringify(payload),
        headers: {
          Authorization: `Bearer ${AIRTABLEAPIKEY}`, // API key
  ...
  if (event.httpMethod === "POST") {
    ...
    if (token) {
      try {
        ...
        const resu = await respormise(JSON.parse(event.body).id);
        console.log(resu);
        return resu;
      } catch (error) {
        return {
